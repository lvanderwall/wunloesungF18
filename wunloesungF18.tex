...
%
\[%
	\uH_2(\us):=\frac{\uQ(\us)}{\uP_2(\us)}=1+\frac{%
		(\us L_1+R_1)(\us R_3C+1)%
	}{%
		\uP_2(\us)%
	}=:1+\frac{\uP_3(\us)}{\uP_2(\us)}=:1+\uH_3(\us)%
\]%
%
Verwende wieder, dass Teilerfremdheit bei Poly.-Div. erhalten bleibt, d.h. \glqq$\uH_2(\us)$ teilerfremd\grqq $\Leftrightarrow$ \glqq$\uH_3(\us)$ teilerfremd\grqq. Prüfe die Teilerfremdheit von $\uH_3(\us)$:
%
\begin{align*}%
...
%
Sei nun $t>t_S$. Suche alle ZRGen. Die STQ $j(t)$ wird kurzgeschlossen und kann weggelassen werden:
\incfigs{a1d}{NWM für $t>t_S$ mit $R_i=R,\: L_i=L$. Ab e) ist $C=0\quad\arr\quad C\:\rightarrow\:\red{\text{LL}}$}{a1d}%

\noindent Mit der Vereinfachung $R_i=R,\: L_i=L$ werden die Teilnetze aus \Fref{a1a} und \Fref{a1d} parallel zu den festen Quellen $j(t)$ bzw. $v(t)$ identisch. Also haben beide NWMe die gleiche Eingangsadmittanz $\uY_{in}$:
\[%
	\uH_4(\jw):=\frac{\uI_{L_1}}{\uV}=-\uY_{in}=-\frac{\uJ}{\uI_j}=-\frac{1}{\uH(\jw)}=-\frac{\uQ(\jw)}{\uP(\jw)}\ceqn{%
		\text{teilerfremd nach A1b)}%
	}%
\]%
%
Sei wieder $\jw\rightarrow\us\im[C]$. Das NWM besitzt für $t>t_S$
\[
	\lr{.}{\begin{aligned}%
		\bullet\quad n_A&=4\text{ diff.-bare Var.: }\vec{x}_A(t)=(i_{L_1},\:i_{L_2},\:i_{L_3},\:u_C)^T(t)\\[2.5mm]%
		%
		\bullet\quad n_R&\geq 1\text{ zustandsred. Glg.: }0=i_{L_1}(t)-i_{L_2}(t)-i_{L_3}(t)%
	\end{aligned}}{\}}\begin{gathered}%
		3=4-1\geq n_A-n_R=n\geq\grad\uP(\us)=3\\[2.5mm]%
		\arr\quad\uP(\us)\text{ liefert alle $n=3$ nat. Freq.!}
	\end{gathered}%
\]
Das NWM aus \Fref{a1d} besitzt genau $n_R=n_A-n=1$ zustandsreduzierende Gleichung:
\[%
	i_{L_1}(t)-i_{L_2}(t)-i_{L_3}(t)=0\quad\arr\quad i_{L_1}(t_S^+)-i_{L_2}(t_S^+)-i_{L_3}(t_S^+)=0%
\]%
%
...
\begin{align*}%
	AG_1:&&0&=U_{D_1S_1}-U_{G_1S_1}-R_1\cancel{I_{G_1}}^{\approx0}\quad\arr\quad U_{D_1S_1}\approx U_{G_1S_1},\qquad I_{D_1}=:f(U_{G_1S_1})\\[2.5mm]%
%
...
\intertext{$AG_1$ und $M_2$ liefern die Kennlinie $AG_2$:}%
...
\end{align*}%
%
Für alle Werte von $U_{G_1S_1}$ aus dem ersten Kennlinienfeld (\Fref{a3a_kennlinienfeld1}) liefert die Kennlinie $AG_1$ die zugehörigen Werte $I_{D_1}$, trage sie in \Tref{a3a_kennlinie} ein:
...
\noindent Mit \Eref{a3a_AG2} berechne einige Punkte von $AG_2$, der nichtlinearen Beziehung $I_{2,DC}(V)$ aus dem Hinweis:
...
\eqn{\label{a3d_MaschenZ}%
	\LGSMat{c}{%
		\jw(L_1+(1\red{+gr_0})L_2)+r_0+R_2+R_T%
...
$R_0$ liegt in Reihe zur STQ $\uI_1$ und kann weggelassen werden; die Rückgewinnung von $\uU_1$ steht in \Fref{a4}. Verwende $\uY_i=\jw C_i+\frac{1}{R_i},\quad G_i:=\frac{1}{R_i}$:
\incfigs{a4}{%
